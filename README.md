...
- 🃏 **点词看释义**：点对话里的单词弹出小卡片（中文释义 + 音标 + **常用搭配** + 发音）。查询顺序：场景词汇 → `glossary.js` → `common-words.js`（~200 基础词，离线可用）→ 在线词典（MyMemory 中文翻译 + 英文释义，仅 https 环境）
- ⭐ **生词本**：卡片上点 ⭐ 收藏，顶部“生词本”随时回顾、朗读、移除（存在浏览器本地）
...
│   ├── common-words.js   # 常用基础词库（~200 词，简明中文）
│   └── scenes/
